...
async function scanComponents(dirs, srcDir) {
  ...
  for (const {path: path$1, pattern, ignore = [], prefix, extendComponent, global, level} of dirs.sort(sortDirsByPathLength)) {
    ...
    for (const _file of await globby2__default['default'](pattern, {cwd: path$1, ignore})) {
      ...
      let componentName = pascalCase(path.basename(filePath, path.extname(filePath)).replace(/^\//g, ""));
      const pathPrefix = pascalCase(path.relative(path$1, path.dirname(filePath)).replace(/\//g, "-"));
      const parentDirName = pascalCase(path.basename(path.dirname(filePath)));
 ...
 - ${resolvedNames.get(componentName)}`);
      ...
      const kebabName = lodash.kebabCase(componentName);
